...
from rows =
    List.map Array.fromList rows |> Array.fromList


indexedRowsMap : (Int -> Row a -> b) -> Grid a -> List b
indexedRowsMap f grid =
    Array.indexedMap f grid |> Array.toList


indexedCellsMap : (Int -> a -> b) -> Row a -> List b
indexedCellsMap f row =
    Array.indexedMap f row |> Array.toList


update : (a -> a) -> Grid a -> Coordinates -> Grid a
update f grid coordinates =
...
set grid coordinates cell =
    let
        ...
        setRowToGrid y g row =
            Array.set y row g

        setCellToRow : Int -> a -> Row a -> Row a
        setCellToRow x c row =
            Array.set x c row
    in
    ...
    let
        ...
        subArray start end array =
            let
                ...
                e =
                    if end < Array.length array then
                        end
...
replaceAll : (a -> b) -> Grid a -> Grid b
replaceAll map grid =
    Array.map (\row -> Array.map map row) grid


indexedReplaceAll : (Coordinates -> a -> b) -> Grid a -> Grid b
indexedReplaceAll cellMap grid =
    let
        rowIndexReplaceAll : (Coordinates -> a -> b) -> Int -> Row a -> Row b
        rowIndexReplaceAll cMap y row =
            Array.indexedMap (\x cell -> cMap { x = x, y = y } cell) row
    in
    Array.indexedMap (rowIndexReplaceAll cellMap) grid


replaceNeighbours : (a -> a) -> Grid a -> Coordinates -> Grid a
replaceNeighbours mapper grid coordinates =
    let
        ...
        mapperForNeighbours m center =
            \cellCoordinates cell ->
                if isNeighbour center cellCoordinates then
                    m cell

                else
                    cell
    in
    indexedReplaceAll (mapperForNeighbours mapper coordinates) grid
